...
const NAMESPACED_DICTIONARY = {
  common: {
    ...
    clinic: 'Clinic',
    // Clinical Profiles
    drChalapathiRao: 'Dr. L. Chalapathi Rao, MD (Gen. Med)',
    drVineetha: 'Dr.Vineetha, MS, DGO',
    drKishor: 'Dr. Kishor, MD, DM (Cardio)',
    drMuraliKrishna: 'Dr. M. Murali Krishna, MBBS',
    drKSrinivas: 'Dr. K. Srinivas, MD (Endo)',
    drBRaghavaReddy: 'Dr. B. Raghava Reddy, MS (Ortho)',
    // Specializations
    specializationGynecology: 'Gynecology',
    specializationCardiology: 'Cardiology',
    specializationGeneralPractice: 'General Practice',
    specializationEndocrinology: 'Endocrinology',
    specializationOrthopedics: 'Orthopedics',
    specializationGeneralMedicine: 'General Medicine'
  },
  auth: {
  ...
  booking: {
    ...
    confirmed: 'Confirmed',
    time: 'Time',
    alreadyHasApt: 'You already have an active appointment. Please complete it before booking another.',
  ...
  schedule: {
    ...
    consultationLoad: 'Current Consultation Load',
    activeConsultations: 'Active Consultations',
    bookedSlotsPeriod: 'Booked Slots for the current period',
    noUpcomingAppts: 'No upcoming appointments scheduled',
    calendarClear: 'Your calendar is currently clear',
  },
  dynamic_keys: {
    'dr. l. chalapathi rao': 'Dr. L. Chalapathi Rao',
    'dr. l. chalapathi rao, md': 'Dr. L. Chalapathi Rao, MD',
    'dr. l. chalapathi rao, md, (gen. med)': 'Dr. L. Chalapathi Rao, MD (Gen. Med)',
    'dr. l. chalapathi rao, md (gen. med)': 'Dr. L. Chalapathi Rao, MD (Gen. Med)',
    'dr.vineetha, ms, dgo': 'Dr.Vineetha, MS, DGO',
    'dr. kishor, md, dm (cardio)': 'Dr. Kishor, MD, DM (Cardio)',
    'dr. m. murali krishna, mbbs': 'Dr. M. Murali Krishna, MBBS',
    'dr. k. srinivas, md (endo)': 'Dr. K. Srinivas, MD (Endo)',
    'dr. b. raghava reddy, ms (ortho)': 'Dr. B. Raghava Reddy, MS (Ortho)',
...
const LANGUAGES = ['en', 'hi', 'te', 'ta', 'mr', 'bn', 'kn', 'ml', 'gu', 'pa'];

async function seed() {
  console.log('🚀 Starting Multilingual Translation Seeding...');
  ...
  for (const lang of LANGUAGES) {
    ...
    for (const [ns, dict] of Object.entries(NAMESPACED_DICTIONARY)) {
      const keys = Object.keys(dict);
      const values = Object.values(dict);
      
      process.stdout.write(`  - Namespace [${ns}] (${keys.length} keys)... `);

      if (lang === 'en') {
        const translatedPack = {};
        keys.forEach((key, i) => {
          translatedPack[key] = values[i];
          flatDictionary[key] = values[i];
        });
      ...
      // Removed manual overrides as namespaced keys and ML translation are sufficient.
      ...
      try {
        ...
        const translatedValues = response.data.translations || [];
        const translatedPack = {};
        
        keys.forEach((key, i) => {
          const val = translatedValues[i] || values[i];
          translatedPack[key] = val;
        ...
        keys.forEach((key, i) => {
          if (!flatDictionary[key]) flatDictionary[key] = values[i];
        });
      }
      console.log(`[${ns}] Total Flat Keys: ${Object.keys(flatDictionary).length}`);
    }

    if (Object.keys(flatDictionary).length > 0) {
      const flatFile = path.join(langDir, `global.json`);
      fs.writeFileSync(flatFile, JSON.stringify(flatDictionary, null, 2));
      console.log(`✨ Completed all namespaces for [${lang.toUpperCase()}]`);
    } else {
      console.log(`⚠️ Skipped writing global.json due to empty data.`);
    }
  }

  console.log('\n🏁 SEEDING COMPLETE! All translation cache files generated.');
}
